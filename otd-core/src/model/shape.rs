//! Shape definition for custom contours and non-rectangular pieces.

use super::Cut;
use serde::{Deserialize, Serialize};

/// Custom shape contour defined by a series of cut segments.
...
impl Shape {
    ...
    /// Calculate the path length from a starting cut index until a discontinuity.
    ///
    /// A discontinuity occurs when:
    /// - Distance between end of cut and start of next > d_min_cont
    /// - Angle change between cuts >= min_angle
    /// - It's the last cut
    pub fn path_length_from(&self, start_index: usize, d_min_cont: f64, min_angle: f64) -> f64 {
        let mut total_length = 0.0;

        for i in start_index..self.cuts.len() {
            let cut = &self.cuts[i];
            if !cut.active {
                continue;
            }
            ...
            // Check if this is a discontinuity point
            if i < self.cuts.len() - 1 {
                let next_cut = &self.cuts[i + 1];
                if next_cut.active {
                    let distance = distance_2d(cut.xf, cut.yf, next_cut.xi, next_cut.yi);
                    let angle_diff =
                        angle_min(cut.final_angle_degrees(), next_cut.initial_angle_degrees());
...
fn angle_min(angle1: f64, angle2: f64) -> f64 {
